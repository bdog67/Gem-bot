...
async function jupSwap(inMint,outMint,amtLamports,slipBps=1500){
  ...
  const sd=await post(JUP_SWAP,{quoteResponse:q,userPublicKey:walletPubkey,wrapAndUnwrapSol:true,prioritizationFeeLamports:100000,dynamicComputeUnitLimit:true,dynamicSlippage:{maxBps:3000}});
  if(sd.error)throw new Error('Jupiter swap: '+sd.error);
  const txBytes=Buffer.from(sd.swapTransaction,'base64');
  const numSigs=txBytes[0],msgOffset=1+(numSigs*64),msgBytes=txBytes.slice(msgOffset);
  ...
  for(let i=0;i<30;i++){
    await sleep(1000);
    try{const st=await rpc('getSignatureStatuses',[[txSig]]);const sv=st?.value?.[0];if(sv?.err)throw new Error('Tx failed:'+JSON.stringify(sv.err));if(sv?.confirmationStatus==='confirmed'||sv?.confirmationStatus==='finalized')break;}catch(e){if(e.message.includes('Tx failed'))throw e;}
  }
...
function dashPage() {
  ...
  const gemRows=lastScanStats.runners.slice(0,8).map(r=>{
    ...
    const signal=r._signal||getBestBuyPoint(r);
    const signalBadge=signal?`<span style="font-size:9px;padding:1px 5px;border-radius:4px;background:${signal.type==='PULLBACK'?'rgba(0,232,122,.15)':signal.type==='VOL_BREAKOUT'?'rgba(139,92,246,.15)':signal.type==='BSR_FLIP'?'rgba(255,176,32,.15)':'rgba(59,158,255,.15)'};color:${signal.type==='PULLBACK'?'#00e87a':signal.type==='VOL_BREAKOUT'?'#8b5cf6':signal.type==='BSR_FLIP'?'#ffb020':'#3b9eff'};margin-left:4px">${signal.type==='PULLBACK'?'📍PB':signal.type==='VOL_BREAKOUT'?'💥VOL':signal.type==='BSR_FLIP'?'🔄BSR':'📈ACC'}</span>`:'';
    return`<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #1a2332;font-size:11px">
      <span><b>${r.baseToken?.symbol||'?'}</b>${signalBadge}</span>
  ...
  // Watchlist: top trending with buy signals
  const watchEntries=[...watchlist.entries()]
    .filter(([,w])=>w.seenCount>=3&&w.buySignal)
    .sort((a,b)=>(b[1].buySignal?.strength||0)-(a[1].buySignal?.strength||0))
    .slice(0,6);
  const watchRows=watchEntries.map(([mint,w])=>{
    const snap=w.snapshots.at(-1)||{};
    const sig=w.buySignal;
    const sigColor=sig.type==='PULLBACK'?'#00e87a':sig.type==='VOL_BREAKOUT'?'#8b5cf6':sig.type==='BSR_FLIP'?'#ffb020':'#3b9eff';
    const sigLabel=sig.type==='PULLBACK'?'📍 Pullback':sig.type==='VOL_BREAKOUT'?'💥 Vol Breakout':sig.type==='BSR_FLIP'?'🔄 BSR Flip':'📈 Accumulation';
    return`<div style="padding:7px 0;border-bottom:1px solid #1a2332">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
        <span style="font-weight:700">${w.symbol}</span>
        <span style="font-size:10px;font-weight:700;color:${sigColor}">${sigLabel}</span>
        <span style="font-size:10px;color:#4a6080">${sig.strength}/100 strength</span>
      </div>
      <div style="font-size:10px;color:#4a6080">${sig.msg} · seen ${w.seenCount}×</div>
    </div>`;
...
<div class="dex">
  <div style="display:flex;justify-content:space-between;margin-bottom:7px"><span style="font-size:11px;font-weight:700;color:#8b5cf6">📡 DEXSCREENER — 12 SOURCES</span><span style="font-size:10px;color:#4a6080">${lastScanStats.filtered} gems · ${watchlist.size} watching</span></div>
  <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px">
    ${['Boosts','New','Meme','Moon','Raydium','Pump','Trending','Viral','100x','New','WEN','Gem'].map(s=>`<span style="font-size:10px;padding:2px 6px;border-radius:8px;background:rgba(0,232,122,.08);border:1px solid rgba(0,232,122,.2);color:#00e87a">${s}</span>`).join('')}
  </div>
  ${gemRows||'<div style="color:#4a6080;font-size:11px;text-align:center;padding:6px">Start the bot to see live gems</div>'}
</div>

<div style="background:#0d1117;border:1px solid rgba(255,176,32,.25);border-radius:8px;padding:9px 12px;margin-bottom:10px">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
    <span style="font-size:11px;font-weight:700;color:#ffb020">📍 BUY SIGNALS — Trending Watchlist</span>
    <span style="font-size:10px;color:#4a6080">${watchEntries.length} active signals</span>
  </div>
  <div style="font-size:10px;color:#4a6080;margin-bottom:8px">Tokens watched across multiple cycles with confirmed buy points</div>
  ${watchRows}
</div>
